...
Future<String?> getUid({required String edrpou}) async {
  var url = Uri.parse('https://vkursi.pro/card/$edrpou');

  // Строка с куками, которые вы хотите добавить в запрос

  // Создание HTTP GET запроса с куками
  Response? response;
  try {
  ...
  if (response.statusCode == 200) {
    RegExp regExp = RegExp(r"/card/relations\?id=([a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12})");
    var match = regExp.firstMatch(response.body);
    final id = match?.group(0)?.replaceFirst('/card/relations?id=', '');
    return id;
  } else {
    return null;
  }
